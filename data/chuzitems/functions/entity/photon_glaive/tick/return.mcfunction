...
# 持ち主の方を向く
    execute if entity @s[scores={ChuzItems.Projectile.Charge=..1}] facing entity @e[type=marker,tag=Chuz.BodyMarker,sort=nearest,limit=1] eyes positioned ^ ^ ^-100 rotated as @s positioned ^ ^ ^-200 facing entity @s eyes positioned as @s run tp @s ^ ^ ^ ~ ~
    execute if entity @s[scores={ChuzItems.Projectile.Charge=2..}] facing entity @e[type=marker,tag=Chuz.BodyMarker,sort=nearest,limit=1] feet run tp @s ~ ~ ~ ~ ~
