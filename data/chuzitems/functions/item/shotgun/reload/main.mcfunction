...
# ざくざくと弾を込める
    ...
    execute if score @s ChuzItems.Reload matches 30.. in overworld run function chuzitems:item/shotgun/reload/finish
...
# ジャストリロード失敗
    execute unless score @s[tag=!ChuzItems.ReloadFailed,scores={ChuzItems.FungusStick=0..}] ChuzItems.Reload matches 9..10 run function chuzitems:item/shotgun/reload/justreload_failed

# ジャストリロード
    execute if score @s[tag=!ChuzItems.ReloadFailed,scores={ChuzItems.FungusStick=0..}] ChuzItems.Reload matches 9..10 run function chuzitems:item/shotgun/reload/justreload_success