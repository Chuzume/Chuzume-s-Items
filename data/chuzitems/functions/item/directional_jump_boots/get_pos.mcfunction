...
# 1tick前の座標を記録
    execute store result score $Y Chuz.Temporary run scoreboard players get @s Chuz.PosSaveY
    
# いまのを記録
    execute store result score @s Chuz.PosDiffY run data get entity @s Pos[1] 100

# 1tick後に保存しておく
    execute store result score @s Chuz.PosSaveY run data get entity @s Pos[1] 100
