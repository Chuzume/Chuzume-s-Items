...
# ざくざくと弾を込める
    ...
    # ルールが弾使うモードなら弾を減らす
        execute if score @s ChuzItems.Reload matches 3.. if score $Gamerule.AmmoRequire Chuz.Rule matches 1 run clear @s chain_command_block{ChuzData:{ItemID:Shotshell}} 1
    # 弾を増やす
        execute if score @s ChuzItems.Reload matches 3.. in overworld run function chuzitems:item/add_ammo
