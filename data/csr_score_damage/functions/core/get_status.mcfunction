#> csr_score_damage:core/get_status
#
# 計算に必要なMobの情報を取得する
#
...
# 防御力
    execute if data storage csr_score_damage: {BypassArmor:0b} store result score $defensePoints ScoreDamageCore run attribute @s generic.armor get 100
    execute if data storage csr_score_damage: {BypassArmor:1b} run scoreboard players set $defensePoints ScoreDamageCore 0
# 防具強度
    execute if data storage csr_score_damage: {BypassArmor:0b} store result score $toughness ScoreDamageCore run attribute @s generic.armor_toughness get 100
    execute if data storage csr_score_damage: {BypassArmor:1b} run scoreboard players set $toughness ScoreDamageCore 0
