...
# Mobへ代入
    ...
    # プレイヤー、またはドラゴンでないなら、ヘルス0でキル
        execute unless entity @s[type=player] unless entity @s[type=ender_dragon] if score $SubtractedHealth ScoreDamageCore matches ..0 run function craftsman_arms:hitreact 
    ...
    # ヘルスが1以上ならUUID攻撃
        execute unless entity @s[type=player] unless entity @s[type=ender_dragon] if score $SubtractedHealth ScoreDamageCore matches 1.. run function craftsman_arms:uuid_hit_no_kb
    ...
    # プレイヤーかつヘルス0なら死亡メッセージ
        execute if entity @s[type=player] if score @s ScoreToHealth matches ..0 run function craftsman_arms:message/death/bullet
# 演出
